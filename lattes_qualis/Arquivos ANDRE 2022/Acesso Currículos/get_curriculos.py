...
try:
	...
	professors = pd.read_csv("Professores.csv", sep=";", encoding='iso-8859-1') 
...
for pos, professor in enumerate(professors["Nome"]): # Iterates through the professors
	...
	if "lattes" in str(link):
		...
		os.rename(f"temp/{professor}.xml", f"../Curriculos/{professor}.xml") # Put the xml file into the folder "Currículos"
